...
class Song {
  ...
  final IconData icon;
  const Song(this.title, this.artist, this.icon);
}
...
class _SettingsState extends State<SettingsPage>{bool notification=true,dark=true; @override Widget build(BuildContext context)=>Scaffold(backgroundColor:const Color(0xFF08051F),appBar:AppBar(backgroundColor:Colors.transparent,title:const Text('Settings')),body:ListView(padding:const EdgeInsets.all(20),children:[Glass(child:Column(children:[_row(context, Icons.security_rounded,'Account & Security',()=>_msg(context, 'Account protected')),const Divider(color:Colors.white10),_switch('Notifications',notification,(v)=>setState(()=>notification=v)),const Divider(color:Colors.white10),_switch('Dark Mode',dark,(v)=>setState(()=>dark=v)),const Divider(color:Colors.white10),_row(context, Icons.language_rounded,'Language',()=>_msg(context, 'English')),const Divider(color:Colors.white10),_row(context, Icons.privacy_tip_outlined,'Privacy Policy',()=>_msg(context, 'Your privacy matters')),const Divider(color:Colors.white10),_row(context, Icons.help_outline_rounded,'Help & Support',()=>_msg(context, 'Support is ready')),const Divider(color:Colors.white10),_row(context, Icons.info_outline_rounded,'About App',()=>_msg(context, 'Karaoke • Version 1.0.0'))]))])); }
Widget _row(BuildContext context, IconData i,String t,VoidCallback f)=>ListTile(onTap:f,contentPadding:EdgeInsets.zero,leading:Icon(i,color:const Color(0xFFFF63D7)),title:Text(t),trailing:const Icon(Icons.chevron_right_rounded,color:Colors.white54));
...
void _msg(BuildContext context,String s)=>showDialog(context:context,builder:(_)=>AlertDialog(title:const Text('Karaoke'),content:Text(s),actions:[TextButton(onPressed:()=>Navigator.pop(context),child:const Text('OK'))]));
